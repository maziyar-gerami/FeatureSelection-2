function [fitnesses,FeatIndex,Out] = FitFunc(pop,nVar,Data)
    ...
    PredClass=knnclassify(X1(round(0.6*size(X1 , 1))+ 1:end  , :), X1(1:round(0.6*size(X1 , 1)) , :)  , Y1(1:round(0.6*size(X1 , 1)) , :) , 1);
